...
export default function Pagination({
  ...
  const totalBlock = Math.ceil(
    parseFloat(Number(totalPages / pageBlock).toString())
  );
  const currentBlock = Math.ceil(
    parseFloat(Number(currentPage / pageBlock).toString())
  );
  const currentBlockPrev =
    currentBlock > 1 ? (currentBlock - 1) * pageBlock : 1;
  const currentBlockNext =
    currentBlock < totalBlock ? currentBlock * pageBlock + 1 : totalPages;
...
function PaginationStartEnd({
  ...
  const icon = direction === "first" ? "처음" : "마지막";
